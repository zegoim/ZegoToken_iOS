...
@implementation ZegoTokenResult
...
+ (ZegoTokenResult*) generateToken:(unsigned int)appid userID:(NSString*)userID  secret:(NSString*)secret
{
    return [ZegoToken generateToken:appid userID:userID secret:secret effectiveTimeInSeconds:86400 payload:@""];
}


+ (ZegoTokenResult*) generateToken:(unsigned int)appid userID:(NSString*)userID  secret:(NSString*)secret effectiveTimeInSeconds:(long long)effectiveTimeInSeconds payload:(NSString*)payload;
{
    ErrorInfo* errorInfo =[[ErrorInfo alloc] init];
    ZegoTokenResult* result = [[ZegoTokenResult alloc] init];
    ...
    ZEGO::SERVER_ASSISTANT::ZegoTokenResult tokenResult = ZEGO::SERVER_ASSISTANT::ZegoServerAssistant::GenerateToken04(appid,userID.UTF8String,secret.UTF8String,effectiveTimeInSeconds,playloadJson.UTF8String);
